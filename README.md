...
- **No new shortcut to learn** — Slickly watches your macOS screenshot folder and reacts to **⌘⇧4 / ⌘⇧3 / ⌘⇧5** automatically.
...
- **Optional manual hotkey** for when you'd rather use Slickly's own region selector (off by default).
...
The first time you press the hotkey, macOS will ask for **Screen Recording** permission for the Slickly app — grant it in *System Settings → Privacy & Security → Screen Recording*.
...
> Slickly resolves your screenshot folder via `defaults read com.apple.screencapture location` (falling back to `~/Desktop`). Anything that drops a fresh `.png` there will be picked up.
...
The optional manual hotkey (Settings → "Manual hotkey") still works the old way: it calls `capture_screenshot`, which spawns `/usr/sbin/screencapture -i -x -t png <tmpfile>` for Slickly's own interactive region selector.

### Where your data lives

```
~/Library/Application Support/app.slickly.desktop/
...
- The watcher only fires when macOS writes a screenshot **to disk** — pressing `Ctrl+⌘⇧4` (copy-to-clipboard) won't trigger Slickly. Either drop the Ctrl or use the manual hotkey for clipboard-only flows.
